...
| **FR-3** Feature resolution, precedence, coverage report, idempotence | `tests/test_resolution.py` (all 18 `test_fr3_*`); `tests/test_services.py::test_fr3_analyze_is_idempotent`; `tests/test_cli.py::test_fr14_analyze_reports_coverage`; `tests/test_api.py::test_fr13_analyze_reports_coverage` | covered (not gated) |
...
| **FR-6** Pairwise transition score, weights, normalization | **metrics M2 `pair_ranking_auc` (≥ 0.90), M2b anti-gaming margin (≥ 0.10), M3 `component_monotonicity` (= 1.00)** + `tests/test_scoring.py` (all 27 `test_fr6_*`, incl. `::test_fr6_weight_normalization`, `::test_fr6_invalid_weights_rejected`, `::test_fr6_d12_seamless_calibration`) | covered — gated |
...
| **FR-8 NFR** n = 500 reorder ≤ 60 s | `tests/test_optimizer.py::test_fr8_perf_smoke` | covered — **slow-marked, excluded from the default suite** and not eval-gated (EVALS.md §7). Run it with `uv run pytest flowlist/ -m slow`. |
...
| **FR-13** API | `tests/test_api.py` (26 tests, incl. `::test_fr13_error_catalog_is_complete` — every code in the FR-13 catalog is reachable and mapped — and `::test_fr13_openapi_lists_every_documented_route`) | covered (not gated) |
