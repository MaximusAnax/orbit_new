...
| FR-9 | Judge pass: cp loss, win model, severity, book exclusion, ACPL, accuracy, key moments | `test_judge.py::test_fr9_*` (22 tests incl. black-perspective sign check, book exclusion, cap, plain-mean accuracy), M4 ≥ 0.90 / M4r ≥ 0.75 (`test_gate_m4*_fr9`), `test_services.py::test_fr9_the_judge_pass_always_runs_and_is_the_rating_basis` | covered |
...
| FR-14 | REST API surface | `test_api.py` (18 tests over every sketched endpoint incl. 409/422/404 mapping), `test_threading.py` (6 tests: the *production* wiring — `create_app()` with no pinned service — driven by concurrent threads against a real SQLite file, plus the store-level shared-connection and single-in-progress race guards) | covered |
| FR-15 | CLI surface | `test_cli.py` (13 tests: init, levels, profile, interactive play incl. resume, games, import, analyze, report, error exits) + hardening-pass manual end-to-end run of every documented command (44 invocations against one real SQLite database — REVIEW.md H6) | covered |
...
Two suites carry no single FR because they cover the substrate every FR sits
on, so they are listed here rather than in the table above:

| Suite | What it covers |
|---|---|
| `test_models.py` (23 tests) | DATA_MODEL.md's model-level invariants, enforced by the pydantic validators rather than by any caller: `Level` margin/blunder-probability coupling and immutability, `RatingState` RD bounds and judged-games/EWMA agreement, `Game`'s played-vs-imported field sets, derived `rated`, status↔`result_score`↔termination consistency, override sanity, move-record colour alternation, `CpuMeta` injection↔roll coupling, `MoveAnalysis` derived-and-capped `cp_loss`, Δw↔severity consistency, category/motif placement, `GameAnalysis` boundary ordering and ply uniqueness, `Suggestion` evidence and rank bounds |
| `test_adapters.py` (15 tests) | The adapter contracts behind FR-2/FR-16: `InternalAnalyst` protocol conformance, determinism, budget honouring, legal PV, terminal positions and default budget; `CommittedBook` protocol conformance and probe purity; and the offline guarantee — the live Stockfish/Lichess adapters refuse without an explicitly configured binary/flag, and `test_offline_path_never_imports_the_live_modules` proves the eval path never loads them |
