"""FR-15: the FastAPI edge. Thin — parse, delegate to the service, serialize."""

from almanac.api.routes import create_app

__all__ = ["app", "create_app"]


def __getattr__(name: str):
    """Build the default ASGI app lazily so importing the package is side-effect free.

    ``uvicorn almanac.api:app`` resolves here.  The routes live in
    ``api/routes.py`` rather than ``api/app.py`` precisely so that this name is
    not shadowed by a submodule of the same name.
    """
    if name == "app":
        return create_app()
    raise AttributeError(name)
