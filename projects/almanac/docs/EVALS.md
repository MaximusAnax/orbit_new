...
| M6 | ≈ 1.0 (feedback ignored) | ≈ 1.0 | ≤ 0.35 |
...
| M6 feedback_responsiveness | ≤ 0.35 (cohorts ≥ 10 each and `rate(gem) > 0`, else FAIL) + archive-candidate sub-check `== 1.0` | The reflection loop must visibly matter. Derived in §4: correct implementation ≈ 0.0–0.2; removing only the `flat_streak ≥ 2` demotion gives 0.5; removing the whole grade table gives ≈ 1.0. The threshold is set between the correct value and the "half-broken" value, which is what makes it a gate rather than a wish. |
