...
*Measured correction (hardening pass, REVIEW.md H2).* The baselines are
computed live in every run; the a-priori levels above were wrong in three
cells. Measured at seed 7: `random_eligible` M2b = 0.971 (not ≈ 0.1 — the
shared cooldown bookkeeping spreads a uniform pick across the library far
better than the estimate assumed; it still fails the 180-day hard bound with
3 starved entries, and fails M2b's `== 1.0` gate), and M4b = 2.77 / 2.09
(random/fifo, not ≈ 4.0 — the `not_due`-pick exclusion compresses the
baselines' O-dispersion), so **M4b alone does not separate the system from
the baselines**; its discriminating power was demonstrated instead by
mutation (interval-blind LRU inside the real scheduler → 3.57, REVIEW.md
H3). The load-bearing claim is unchanged and measured true: each baseline
fails at least five gates of the set (`random_eligible`: M2a×2, M2b, hard
bound, M3, M5, M6; `fifo_rotation`: M2a×2, M3, M5, M6).
...
| M6 feedback_responsiveness | ≤ 0.60 (cohorts ≥ 10 each and `rate(gem) > 0`, else FAIL) + archive-candidate sub-check `== 1.0` | The reflection loop must visibly matter. **Amended at build time (REVIEW.md B1); the original 0.35 and its derivation are below.** §4's derivation assumed `lambda ≈ 2.5`, but the fixture sizes declared in the same section produce `lambda ≈ 3.2–3.4`, which pushes each dud's demotion-triggering third exposure from ~day 120 to ~day 170 — just inside the `[165, run end]` horizon instead of just outside it. The replacement derivation is empirical, run at the realized `lambda`, and repeats §4's sensitivity check by removing the mechanisms: correct implementation 0.382–0.516 across seeds 7/8/9, flat-streak *tracking* removed — which disables both the demotion and the `flat_streak ≥ 3` archive-candidate signal — 0.668–0.718, whole grade table flattened 0.797–0.883. The gate sits between the correct implementation's *worst* seed and the half-broken variant's *best* seed, so it still fails both degradations at every seed — the property that made the original a gate rather than a wish. *(Hardening correction, REVIEW.md H1: the middle variant was originally mislabeled "demotion removed". Removing only the demotion branch while leaving streak tracking intact measures 0.413–0.563 and passes this gate, because the persona archives the duds the still-live archive-candidate signal flags; that narrower degradation is instead caught deterministically by M7c's independent re-fold, which reports cache≠fold on every demoted entry.)* Superseded: "≤ 0.35 … correct implementation ≈ 0.0–0.2; removing only the demotion gives 0.5; removing the whole grade table gives ≈ 1.0." |
