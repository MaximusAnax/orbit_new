...
| M5 constraints | "3×10 everything, same full-body session daily, no deload, fixed rep range, no RIR" | ≈ 0.47 (fails volume bands, coverage, frequency, deload, RIR ramp, goal rep ranges, ordering, experience scaling) | **= 1.0** | The generator is deterministic and the constraints are stated in SCOPE.md; any violation is a bug, same rationale as Orbit's 100 % safety gates. |
