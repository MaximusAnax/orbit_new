...
- **A. Optimal, constraint-correct redemption-path search.** Given holdings,
  ...
  transfer-time budgets, seat limits, and joint funding across multiple
  bookings from shared balances — and rank the alternatives correctly. A
  search that is merely "usually right" silently loses the user hundreds of
  dollars and cannot be trusted with irreversible moves.
...
credit-score modeling and no card-acquisition advice (see Non-goals 4/5).
...
append-only ledger; `wallet show` lists cards, balances, each balance's
baseline value and best cash floor; edges/options requiring a card I do not
hold are excluded from every plan (M2 validity gate: violations = 0).

**US-2 — Tell me what my points are worth.** As the user, I can ask the value
of any balance and get both the honest floor (best cash-out available *to me*)
and the baseline valuation (what a reasonable redemption achieves), with the
valuation's `as_of` date shown.
*Accept:* `value <program> <points>` returns floor and baseline in dollars
using the documented integer math (M4 gate); the floor reflects card gating
(1.5¢ portal only with the CSR held); output always carries the valuation
`as_of` date.
...
and the top-5 ordering matches (M3 = 1.0); every plan step is executable as
written (amounts respect increments/minimums; M2 = 0 violations).
...
liquidation of some or all balances and get a plan ranked by cash received,
with the value-destroyed warning when cashing out under baseline.
*Accept:* cash plans use only options active for my cards, respect minimums
and increments, and match the oracle's optimum on cash scenarios (in M1);
the combined plan covers every program with a positive balance and an active
cash option.
...
out its risks: transfers are irreversible, this edge takes 2 days and my
booking deadline is in 3, 700 points will be stranded in Flying Blue, this
promo ratio expires next week, the valuation data is 8 months old.
*Accept:* the caveat list is deterministic data (typed caveat codes + filled
templates), asserted per fixture scenario; the stranding caveat's cent value
equals the accounting ground truth (M4); a world dataset older than 180 days
relative to `today` always produces a `stale_world` caveat.
...
against a *different* world version is refused with a re-plan hint.
*Accept:* PlanSet stores `world_version` + content hash; D0 determinism test
(byte-identical recompute) passes; version-mismatch execution returns a
structured error.
...
- **FR-1 World dataset load & validation.** Load the committed rewards world
  ...
  load: unique ids; referential integrity; `increment_from % ratio_from == 0`
  on every edge (so delivered amounts are exact integers); every program has
  a valuation; every award offer's origin/destination resolve in the
  gazetteer and are covered by a reference fare for every month its travel
  window touches; version hash matches file contents. Any violation is a
  named, actionable error; `world validate` and `init` both run it.
...
- **FR-5 Offline goal parsing.** `RuleBasedGoalParser` turns free text into a
  ...
  Missing origin defaults to the profile's home city if set, else
  `ParseError`. Gate M5.
- **FR-6 Offer matching.** Deterministic predicate from `GoalSpec` to award
  ...
  offer `bookable_until ≥ today` when present.
- **FR-7 Funding-path search (hard part A).** For a candidate *booking set*
  (one round-trip offer; an outbound + return pair of one-way offers,
  possibly on different programs; one stay offer; or a portal booking) find
  the **exact minimum-cost funding**: transfer chains of ≤ `max_hops = 2`
  active edges ending at each booking's program, starting from programs with
  positive balances, no program revisited in a chain; per-chain sent-amount
  candidates limited to the lossless lattice {smallest increment-rounded
  amount delivering each needed quantity} ∪ {tier-boundary amounts within
  range} ∪ {full balance}; joint allocation across all bookings in the set
  via branch-and-bound with the admissible lower bound
  `remaining_deficit × min_marginal_cost` and dominance pruning (a partial
  state is pruned when another state covers ≥ the deficit at ≤ the cost with
  ≤ the time); cumulative chain time must fit the goal's `book_by` budget.
  The search is exact by construction; it never approximates silently — if
  the explicit expansion budget (`max_expansions`, default 200,000) is hit
  it raises `SearchBudgetExceeded`. Gates M1/M2.
- **FR-8 Plan objective & value accounting (hard parts A+B).** All money in
  ...
  For a plan taking holdings H0 to H1:
  `gross_value` = Σ bookings' reference value (flight: `pax × ref_fare`;
  stay: `nights × nightly_ref`; portal booking: the same reference fare;
  cash: cash received);
  `cash_outlay` = Σ award fees + Σ transfer fees, where a transfer fee is
  `min(fee_cap_cents, ceil(sent × fee_mcpp / 1000))`;
  ...
  points); transfer fees live in net value, not cpp (documented). Tier
  bonuses: `bonus_to × (sent // per_from)` per transfer step. Portal points:
  `ceil(fare_cents × 1000 / cpp_milli)` rounded up to the option increment.
  Gate M4.
- **FR-9 Ranking, alternatives, verdict (hard part A).** One plan per
  feasible booking set (its optimal funding). Ranking key: flight/stay goals
  by `net_value` desc; cash goals by cash received desc; canonical
  tie-break: fewer total points spent, then fewer steps, then plan
  `signature` (stable hash of ordered step tuples) ascending — total order,
  so output is byte-stable. Top-K returned (default 5), plus the best
  portal/cash comparator appended (flagged `comparator`) if feasible and not
  already present. Plan-set verdict: `book_with_points` when the best plan
  has net_value > 0; `pay_cash_keep_points` when best ≤ 0;
  `insufficient_points` when offers match but none can be funded;
  `no_matching_award` when nothing matches; `cash_plan` for cash goals.
  Gate M3.
- **FR-10 Explanations & caveats (hard part B).** Every step renders a
  ...
  text equal to the stored field (asserted in tests). Typed caveats with
  params: `irreversible_transfer`, `transfer_time_risk` (chain days vs
  book_by), `stranded_points` (amount, program, cent value from FR-8),
  `promo_expiring` (edge, valid_to), `below_baseline` (cash-out under
  baseline), `stale_world` (as_of > 180 days before today),
  `seats_limited`. Caveat production is a pure function of the plan + world
  + today.
- **FR-11 Plan persistence & execution loop.** Computing plans persists an
  immutable `PlanSet` (goal, world_version + hash, today, params) with
  ranked `Plan`s and ordered `PlanStep`s. Executing step k requires steps
  1..k−1 executed, the loaded world version to equal the pinned one, and
  `confirm_irreversible = true` for transfer/book_award steps; execution
  writes the FR-2 ledger entries atomically and stamps `executed_at`
  (caller-supplied time). Re-executing an executed step is a no-op error.
- **FR-12 Cash goal engine.** For `cash` goals: per program (after optional
  filter/cap), choose the best active cashout option (cpp desc, then option
  id) over the redeemable amount respecting `min_points`/`increment`; the
  top plan combines every program with a positive redeemable balance;
  alternatives swap per-program options and may include cash-improving
  transfer chains when the world contains any (the generic FR-7 search runs;
  in the shipped world direct options dominate, and a stress-world fixture
  proves the general path). Below-baseline redemptions carry the caveat.
- **FR-13 Quick valuation.** `value(program, points)` returns baseline value
  (mcpp math) and the user's floor: best active cash-equivalent option's
  value net of quantization (unredeemable remainder valued at 0 for the
  floor), each with the option/valuation provenance. Pure function; used by
  `wallet show` totals.
...
projects/pointsmax/
  src/pointsmax/
    ...
    engine/
      ...
      search.py          # FR-7: chains, candidate lattices, branch-and-bound
      value.py           # FR-8: portfolio-delta net value, realized cpp, fees, tiers
      plan.py            # FR-9/FR-10: ranking, signatures, steps, templates, caveats
...
GET  /wallet                                  # cards held, balances, baseline totals, floors
...
GET  /valuations                              # per program: baseline + this user's floor
...
pointsmax value <program> <points>
...
6. **Fees are per-edge (rate, cap) pairs**, grounded in Amex's excise-tax
   ...
   to US airline programs. Fee math uses ceiling division (conservative).
7. **Tier bonuses are step functions**, grounded in Marriott Bonvoy's
   +5,000 miles per 60,000 points transferred to airlines (on top of 3:1).
   Tier boundaries are explicit candidate amounts in the FR-7 lattice
   because sending exactly 60,000 can beat sending 58,000.
...
9. **Round-trips may mix one-way awards across programs** — standard
   maximizer practice (e.g. outbound on Flying Blue, return on Aeroplan).
   This forces *joint* funding optimization over shared source balances
   (greedy sequential funding is provably suboptimal when two bookings
   compete for the cheapest source); eval scenarios include exactly these
   conflicts.
10. **An award is paid from a single program's balance** (no cross-program
    ...
    *into* that program via transfers.
...
12. **Canonical total ordering** (FR-9) makes ranking byte-stable and lets
    evals demand exact ranking matches instead of fuzzy correlation.
13. **Time is an input.** `today` drives promo windows, month resolution,
    staleness, and `book_by` budgets; `at` timestamps ledger entries. Edges
    (CLI/API) may default from the system clock; the engine never reads it.
14. **Irreversibility is a first-class safeguard.** Every program treats
    ...
    `irreversible_transfer` caveats on every transfer step, and stranding
    costs priced into net value rather than footnoted.
...
16. **Ledger-event-sourced balances** with replay invariant (mirrors
    ChessMentor's rating-event replay test): manual edits, transfers, and
    redemptions all flow through one append-only mechanism, so plan
    execution and reality cannot drift apart silently.
17. **Search exactness strategy:** bounded exhaustive branch-and-bound with
    ...
    candidate lattice (FR-7). The lattice is complete because between two
    increment points the delivered amount is constant (so only
    need-covering minima matter) and tier bonuses only improve at
    boundaries (included explicitly). Exactness is *gated*, not assumed:
    M1 compares against an independent full-lattice brute-force oracle.
18. **Worlds are small by nature.** ~18 programs, ≤ 6 usable chains per
    target, balances ≤ ~2M points, increments ≥ 250 keep the exact search
    well under the 200k-expansion budget (< 1 s per goal expected);
    the budget errors loudly rather than degrade silently (FR-7).
...
20. **Sizing assumption:** implementation lands in ~2,600–3,600 lines
    (engine ≈ 1,500; adapters ≈ 250; store ≈ 350; api ≈ 300; cli ≈ 300;
    models ≈ 300), inside the 2–4k mandate. Scope valves if pressure
    appears: drop the `stay` goal kind (machinery is shared; only data and
    matching shrink) and the `llm` extra — never the search, accounting, or
    eval surface.
